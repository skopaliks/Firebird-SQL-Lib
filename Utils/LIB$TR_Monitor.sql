...
CREATE OR ALTER TRIGGER LIB$TR_Monitor_Start INACTIVE ON TRANSACTION START POSITION 1
AS
DECLARE gn BIGINT;
BEGIN
  -- Just to pass Elekt Labs replication check
...
CREATE OR ALTER TRIGGER LIB$TR_Monitor_Commit INACTIVE ON TRANSACTION COMMIT POSITION 32761
AS
BEGIN
...
CREATE OR ALTER TRIGGER LIB$TR_Monitor_Rollback INACTIVE ON TRANSACTION ROLLBACK POSITION 32761
AS
BEGIN
  IF(RDB$Get_Context('USER_TRANSACTION','LIB$Transactions_Log_bDateUTC') IS NOT NULL)THEN
